...
export async function scoreResumeJobMatch(
  resumeId: string,
  jobId: string
): Promise<{
  resume_id: string;
  ...
  const res = await apiFetch(`/matching/score/${encodeURIComponent(resumeId)}/${encodeURIComponent(jobId)}`);
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    throw new Error(`Failed to score match (status ${res.status}): ${text}`);
  }
  return res.json();
}
