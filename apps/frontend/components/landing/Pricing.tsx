...
export default function Pricing() {
  return (
    <MeshBackground className="py-28">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16 reveal">
          <div className="text-xs font-mono text-primary-foreground/60 uppercase tracking-wider mb-3">/ Pricing</div>
          <h2 className="font-display text-5xl md:text-6xl leading-[1] text-primary-foreground">
            Simple pricing. <em className="text-white/90">Hire-worthy results.</em>
          </h2>
          <p className="mt-5 text-primary-foreground/60 max-w-md mx-auto">
          ...
          {tiers.map((t, i) => (
            <div
              ...
              className={`relative rounded-2xl p-8 flex flex-col transition-all duration-300 reveal delay-${i + 1} ${
                t.featured
                  ? 'bg-foreground text-background border border-primary/30 shadow-[var(--shadow-elevated)] lg:-translate-y-3'
                  : 'bg-card border border-border/50 hover:-translate-y-1 hover:border-primary/40'
              }`}
            >
              {t.featured && (
        ...
        <p className="text-center text-xs font-mono text-primary-foreground/40 mt-10">
